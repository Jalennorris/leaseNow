...
export default {
    ...
    createLease: async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            ...
            const result = await pool.query(
                'INSERT INTO leases (renter_id, property_id, start_date, end_date, monthly_rent, deposit) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
                [renter_id, property_id, start_date, end_date, monthly_rent, deposit]
            );
    ...
    updateLease: async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            // Update lease
            const result = await pool.query(
                'UPDATE leases SET renter_id = $1, property_id = $2, start_date = $3, end_date = $4, monthly_rent = $5, deposit = $6 WHERE lease_id = $7 RETURNING *',
                [renter_id, property_id, start_date, end_date, monthly_rent, deposit, id]
            );
    ...
    getAllLeases: async (req: Request, res: Response): Promise<void> => {
        try {
            // Check cache first
